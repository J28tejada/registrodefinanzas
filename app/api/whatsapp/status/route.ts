...
import { getAllNumbers } from '@/lib/whatsapp/db';
import { connectionState, evolutionConfig, getWebhook, webhookUrl } from '@/lib/whatsapp/evolution';
import { MODEL, TIMEZONE, geminiApiKey } from '@/lib/whatsapp/config';

export const dynamic = 'force-dynamic';

/** Todo lo que el sistema sabe, de una: el diagnóstico no debería requerir logs (§5.6). */
  ...
  if (cfg) {
    ...
    try {
      ...
      const porEventos =
        ...
        false;
      // §6.1: con webhookByEvents en true el webhook nunca recibe nada.
