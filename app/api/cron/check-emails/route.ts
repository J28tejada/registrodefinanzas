...
async function syncUserEmails(userId: string): Promise<{ processed: number; added: number }> {
  const token = await getGmailToken(userId);
  if (!token) return { processed: 0, added: 0 };
  ...
  for (const messageId of toProcess) {
    try {
      ...
      console.error(`Error processing message ${messageId} for user ${userId}:`, err);
    }
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const userId of userIds) {
      try {
        const { processed } = await syncUserEmails(userId);
        totalProcessed += processed;
      } catch (err) {
        console.error(`Cron sync error for user ${userId}:`, err);
      }
    }
