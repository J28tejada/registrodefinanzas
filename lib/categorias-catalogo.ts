/**
 * El catálogo de íconos de las categorías, sin depender de la plataforma.
 *
 * Acá vive la ÚNICA lista: qué claves existen, en qué grupo va cada una y con
 * qué ícono de lucide se dibuja —por su NOMBRE, no por el componente—. La web
 * lo resuelve contra `lucide-react` y el teléfono contra `lucide-react-native`,
 * cada uno importando solo lo que usa.
 *
 * Es lo que impide que las dos apps se separen. Si la clave 'auto' fuera un
 * `Car` en la web y un `Truck` en el teléfono, nadie se daría cuenta hasta
 * verlo; con el nombre declarado en un solo lugar, `scripts/verificar-iconos.mjs`
 * lo detecta antes de que salga.
 */
import { LedgerColor, LEDGER_COLOR_MAP } from './types';

/** Clave -> nombre del ícono en lucide. El mismo en las dos plataformas. */
export const NOMBRES_DE_ICONO: Record<string, string> = {
  // Día a día
...
/** El nombre del ícono genérico: el de una categoría que no eligió ninguno. */
export const NOMBRE_ICONO_POR_DEFECTO = 'Tag';
