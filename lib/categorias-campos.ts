import { COLORES_CATEGORIA, NOMBRES_DE_ICONO } from './categorias-catalogo';
 ...
 *
 * Contra el catálogo compartido y no contra el mapa de la web: el teléfono corre
 * esta misma función antes de escribir en la base, y su mapa de íconos es otro
 * —componentes de `lucide-react-native` en vez de `lucide-react`—. El catálogo
 * es el único que los dos pueden mirar.
 */
...
export function leerIconoYColor(
  ...
  if (b.icon !== undefined) {
    if (b.icon === null || b.icon === '') {
      campos.icon = null;
    } else if (typeof b.icon !== 'string' || !(b.icon in NOMBRES_DE_ICONO)) {
