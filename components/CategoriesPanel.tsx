...
export default function CategoriesPanel() {
  ...
  return (
    <div className="bg-panel border border-t-borde-luz border-linea rounded-2xl p-4 sm:p-5 space-y-5">
      ...
      {GRUPOS.map(({ type, titulo }) => {
        ...
        return (
          <div key={type} className="space-y-3">
            <p className="text-xs text-tinta-2 font-medium">{titulo}</p>
            ...
            {/* Tres columnas en el teléfono y no cuatro: con cuatro la celda
                ...
                >
                  <span className="relative">
                    ...
                    {subDe(cat.id).length > 0 && (
                      <span className="absolute -bottom-0.5 -right-0.5 min-w-[16px] h-4 px-1 rounded-full bg-presionado border border-linea text-4xs text-tinta flex items-center justify-center tabular-nums">
                        {subDe(cat.id).length}
                      </span>
                    )}
                  </span>
                  <span className="text-2xs text-tinta leading-tight text-center line-clamp-2 w-full break-words hyphens-auto">
                    {cat.name}
                  </span>
                </button>
              ))}
              ...
              <button
                ...
                className="flex flex-col items-center gap-1.5 group"
              >
                <span className="w-11 h-11 rounded-full border border-dashed border-linea-fuerte group-hover:border-tinta-2 flex items-center justify-center transition-colors">
                  <Plus className="w-5 h-5 text-tinta-2 group-hover:text-acento transition-colors" />
                </span>
                <span className="text-2xs text-tinta-2 leading-tight">Agregar</span>
              </button>
            </div>
      ...
      <div className="text-xs text-tinta-2 space-y-1.5">
        <p>
...
function EditorDeCategoria({
  ...
  return (
    <div className="border border-linea rounded-xl p-3 space-y-3 bg-fondo">
      ...
      <div className="flex gap-2">
        {cat && (
          <button
            ...
            className="px-3 py-2.5 bg-hundido hover:bg-peligro/85 disabled:opacity-50 text-tinta hover:text-tinta rounded-lg text-sm transition-colors flex-shrink-0"
          >
...
function Subcategorias({
  ...
  return (
    <div className="space-y-2 pt-1 border-t border-linea">
      <p className="text-2xs text-tinta-2 pt-2">
      ...
      <div className="flex flex-wrap gap-1.5">
        {lista.map(sub => (
          <span
            key={sub.id}
            className="flex items-center gap-1 bg-hundido border border-linea-fuerte rounded-lg pl-2.5 pr-1 py-1"
          >
            <span className="text-xs text-tinta">{sub.name}</span>
            <button
              type="button"
              onClick={() => borrar(sub)}
              disabled={ocupado === sub.id}
              aria-label={`Eliminar ${sub.name}`}
              className="p-0.5 text-tinta-2 hover:text-peligro disabled:opacity-50 transition-colors"
            >
              {ocupado === sub.id
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="px-3 py-2 bg-hundido hover:bg-presionado disabled:opacity-50 text-tinta rounded-lg text-sm transition-colors flex-shrink-0"
        >
